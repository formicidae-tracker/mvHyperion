...
//-------------------------------------------------------------------------------------------
int
hyperion_get_device_number( void )
//-------------------------------------------------------------------------------------------
{
    int i;
...
static ssize_t
hyperion_number_devices_show( struct device *class_dev,
                              struct device_attribute *aatr, char *buf )
#else
static ssize_t
hyperion_number_devices_show( struct class_device *class_dev, char *buf )
#endif
//-------------------------------------------------------------------------------------------
{
...
#else
CLASS_DEVICE_ATTR( number_devices, S_IRUGO, hyperion_number_devices_show,
                   NULL );
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION( 2, 6, 26 )
struct device *
hyperion_class_device_create( void *device, dev_t dev_num )
//-------------------------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------------------------
{
    ...
    class_member = device_create( hyperion_class, NULL, dev_num, NULL,
                                  "hyperion-cl%d", device_index );
#else
    class_member = device_create( hyperion_class, NULL, dev_num,
                                  "hyperion-cl%d", device_index );
#endif
    dev_set_drvdata( class_member, (void *)device );
    if( ( err = device_create_file( class_member, &dev_attr_number_devices ) )
        < 0 )
    {
        printk( "Error %d creating hyperion device%d class file: "
                "class_device_attr_number_devices",
                err, device_index );
    }
#else
    ...
    if( ( err = class_device_create_file( class_member,
                                          &class_device_attr_number_devices ) )
        < 0 )
    {
        printk( "Error %d creating hyperion device%d class file: "
